"""Leitura do acervo NORMATIVO do TCE-RJ — deliberações, resoluções, atos.

Vive ao lado do acervo de jurisprudência, no mesmo servidor, mas em banco
separado e com ferramentas próprias. A convivência é deliberada: quem pergunta
se pode exigir visita técnica quer a súmula E a deliberação que a regulamenta.
A separação dos bancos é o que impede que as duas réguas se misturem.
...
class AcervoNormas:
    def __init__(self, caminho: str | Path) -> None:
        ...
        if not self.caminho.exists():
            raise FileNotFoundError(
                f"Acervo normativo não encontrado em {self.caminho}."
            )
    ...
    def cobertura(self) -> dict[str, Any]:
        ...
        # As alterações vêm da tabela de relações, e não de uma busca por data
        # somada a LIKE: aquela varre tudo que MENCIONA o Regimento e pega
        # também quem apenas o regulamenta — a Deliberação 346/2024 disciplina
        # o art. 15 sem alterá-lo. Duas fontes para o mesmo fato dariam duas
        # respostas, e a divergência apareceria só para quem comparasse.
        posteriores = self.conexao.execute(
            "SELECT a.numero, a.ano FROM relacoes r JOIN atos a ON a.id = r.outro_id"
            " WHERE r.ato_id = ? AND r.relacao = 'alterado_por'"
            " ORDER BY a.data", (ri["id"],)).fetchall() if ri else []
        return {
