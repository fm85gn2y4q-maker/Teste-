...
é reproduzível, o rollback é trocar a versão, e um reprocessamento ruim não
altera produção em silêncio.

O hash é calculado em fluxo, e não carregando o arquivo inteiro na memória:
são 266 MB comprimidos, e o contêiner de construção nem sempre tem folga.
...
def baixar(url: str, destino: Path, esperado: str | None = None) -> None:
    ...
    print(f"Acervo em {destino} ({destino.stat().st_size / 1048576:.0f} MB)",
          file=sys.stderr)
