"""Entrada do servidor MCP dos pareceres da PGE-RJ.

    python -m pareceres                  # stdio, para o Claude Desktop
    python -m pareceres --http           # HTTP em 127.0.0.1:8766
"""
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        prog="python -m pareceres",
        description="Servidor MCP sobre os pareceres da PGE-RJ em contratações, "
                    "acordos e parcerias.",
    )
    parser.add_argument("--http", action="store_true",
                        help="serve por HTTP em vez de stdio")
    parser.add_argument("--host", default=os.environ.get("PARECERES_HOST", "127.0.0.1"))
    parser.add_argument("--porta", type=int, default=int(os.environ.get("PORT", "8766")))
    ...
    do_ambiente = os.environ.get("PARECERES_DOMINIOS", "")
    ...
    if args.http:
        ...
        print(f"Pareceres PGE-RJ em http://{args.host}:{args.porta}/mcp  ({alcance})",
              file=sys.stderr)
